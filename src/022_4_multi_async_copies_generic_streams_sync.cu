...
//
//This version replaces the cudaDeviceSycnhronize() call
//after the async memcopies with a set of events used
//to synchronize only the dependent streams i.e.
//if gpus 0 1 2 3 are used with 0 reading from 1 and
//2 reading from 3 then the kernel on 0 waits for
//memcopy on 1 and kernel on 2 waits for memcopy on 3
...
int main(int argc, char** argv) {
...
#ifdef PEER_ACCESS
    vector< cudaEvent_t > events(NUM_DEVICES, cudaEvent_t());
    //create one event per gpu
    for(int d = 0; d != NUM_DEVICES; ++d) {
        const int gpu = gpus[d];
        err = cudaSetDevice(gpu);
        assert(err == cudaSuccess);
        err = cudaEventCreate(&events[d]);
        assert(err == cudaSuccess);
    }
    ...
    for(int d = 0; d != NUM_DEVICES; ++d) {
        ...
        //to be completed so that data are available at time of kernel execution    
        cudaStreamWaitEvent(streams[d], events[MapKey(d, NUM_DEVICES)], 0);    
