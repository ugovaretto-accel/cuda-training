...
#ifdef PEER_ACCESS
__device__
Int8 Reduce(Int dest, Int8 src) {
    return dest + src;
}
__global__
void AllGather(Int8** srcs, int N, Int8* dest) {
    size_t i = blockDim.x * blockIdx.x + threadIdx.x;
    for(int s = 0; s != N; ++s) dest[i] = Reduce(dest[i], srcs[s][i]);
}
#else
__global__
...
template < typename T >
void AllocateDevicePtrArray(T**& ptr, const vector< T* >& buffers) {
    cudaError_t err = cudaMalloc(&ptr, buffers.size() * sizeof(void*));
    assert(err == cudaSuccess);
    const size_t PTRSIZE = sizeof(void*);
    for(int i = 0; i != buffers.size(); ++i) {
        err = cudaMemcpy(ptr + i * PTRSIZE, buffers[i], PTRSIZE);
        assert(err == cudaSuccess);
    }
}

template < typename T >
void DeallocateDevicePtrArray(T** ptr, int N) {
    cudaError_t err = cudaError_t();
    for(int i = 0; i != N; ++i) {
        err = cudaFree(ptr + i * PTRSIZE);
        assert(err == cudaSuccess);
    }
    err = cudaFree(ptr);
    assert(err == cudaSuccess);
...
int main(int argc, char** argv) {
    ...
    for(int d = 0; d != NUM_DEVICES; ++d) {
        ...
        assert(err == cudaSuccess);
#ifdef PEER_ACCESS
        AllocateDevicePtrArray(devBuffers[d], deviceBuffers[d]);   
#endif            
    }
...
#ifdef PEER_ACCESS
    vector< Int8** > devBuffers(NUM_DEVICES, (Int8**)(0));
#endif
    for(int d = 0; d != NUM_DEVICES; ++d) {
        const int gpu = gpus[d];
        err = cudaSetDevice(gpu);
...
#ifdef PEER_ACCESS
        AllocateDevicePtrArray(devBuffers[d], deviceBuffers);       
        AllGather<<< BLOCK_SIZE, THREAD_BLOCK_SIZE, 0, streams[d] >>>(
                devBuffers[d], deviceBuffers[d], KERNEL_ENABLED_OPTION);
#else
    ...
    for(int d = 0; d != NUM_DEVICES; ++d) {
        ...
        assert(err == cudaSuccess);
  #ifdef PEER_ACCESS      
        DeallocateDevicePtrArray(devBuffers[d]);
  #endif      
    }
    err = cudaDeviceReset();
    assert(err == cudaSuccess);
